...
PSLRPMore := function(p)
	...
	local G,conjClasses,rep,maximals,max,max3s,genPos,cl,maxWithRep,triple,toTest,test,int1,int2,int3,R,int1MinusR,int2MinusR,int3MinusR,output,x,y,z,outs;
	...
	for test in toTest do
		...
		for triple in max3s do
			SortBy(triple, StructureDescription);
...
#order3s := function(max)
#	# This finds all primes which have witnesses to failure of order 3.
#	local p;
#
#	for p in Primes do
#		if p<max and p>5 and conjecture(p)=false and not(p=7) and not(p=11) and not(p=19) and not(p=31) then
#			Print(p, ": ", PSLRPv8(p), "\n");
#		fi;
#	od;
#
...
RJust:=function(n,x)
    # This function is only used for formatting output of PSLRPv7Test. It was taken from R.K. Dennis's file utility5.gap.
