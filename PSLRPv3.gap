...
PSLRPv3 := function(p)
	# This function takes in a prime p. It first finds all elements of prime order in PSL(2,p). It then stores all distinct conjugacy classes of those elements. Next, looping 	# over all conjugacy classes, it picks a representative from each conjugacy class and stores it. It then calculates all triples of maximal subgroups of PSL(2,p) which are in 	# general position which also contain the representative of the conjugacy class by combining the functions PSLMax, Combinations, and genPosList. Combinations(PSLMax(p), 3). 	# It then checks if the intersection of any three maximal subgroups in general position is trivial. If it is trivial for all triples of maximal subgroups in general position 	# containing representatives of conjugacy classes of elements of prime order, then the replacement property holds, as per a result from R. K. Dennis and Dan Collins (and 	# the fact that the set of witnesses to failure is characteristic - the prime order part I still need to think about). Thus, the output of the function is true if PSLRP 	# satisfies the replacement property for the given input data and false otherwise.

	local G, conjClasses, rep, maximals, max, max3s, genPos, cl, maxWithRep, triple, primeOrds, g, x;

	G := PSL(2,p);
	conjClasses := ConjugacyClasses(G); # This is where we will store distinct conjugacy classes
	maximals := PSLMax(p);
	maxWithRep := []; # This is where we will store all maximal subgroups containing a given representative of a given conjugacy class
	primeOrds := []; # This is where we will store all elements of prime order
	toTest := [];
	
	for g in G do
		if IsPrimeInt(Order(g)) then
			Add(primeOrds, g);
		fi;
	od;
	
	for x in primeOrds do
		cl := ConjugacyClass(G, x);
		if not(cl in conjClasses) then
			Add(conjClasses, cl);
		fi;
	od;
